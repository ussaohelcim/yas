-- Stolen from adn/snkrx
-- MIT https://github.com/a327ex/SNKRX
...
function math.rotate_point(x, y, r, ox, oy)
  return x * cos(r) - y * math.sin(r) + ox - ox * cos(r) + oy * math.sin(r),
      x * math.sin(r) + y * cos(r) + oy - oy * cos(r) - ox * math.sin(r)
end
...
function math.sine_in_out(t)
  if t == 0 then return 0
  elseif t == 1 then return 1
  else return -0.5 * (cos(t * PI) - 1) end
end
...
function math.sine_out_in(t)
  ...
  else return -0.5 * cos((t * 2 - 1) * PI2) + 1 end
end
...
---Returns an angle from a NORMALIZED VECTOR (this doesnt normalize)
---@return number
function math.normalizedVector2ToAngle(x, y)
...
function math.checkCollisionCircles(c1x, c1y, c1r, c2x, c2y, c2r)
  local dx = c2x - c1x
  local dy = c2y - c1y
